...
func NewERC20Manager(cc *database.Currency, privateKeyHex string) (*ERC20Manager, error) {
	log := logger.GetLogger().WithFields(map[string]interface{}{
		"rpc_url": cc.Blockchain.RpcURL,
		"symbol":  cc.Symbol,
	})
	...
	ci := big.NewInt(int64(cc.Blockchain.ChainID))

	log.WithFields(map[string]interface{}{
		"admin_addr": addr.Hex(),
		"chain_id":   ci.String(),
	}).Info("ERC20Manager initialized")
...
func (em *ERC20Manager) sendTxWithMethod(pk *ecdsa.PrivateKey, to common.Address, funcName ERC20Type, gasLimit uint64, gasPrice *big.Int, nonce uint64, am *big.Int, ins ...interface{}) (common.Hash, error) {
	...
	log.WithField("txHash", txHash.Hex()).Info("sendTxWithMethod succeeded")
	return txHash, nil
}
...
func (em *ERC20Manager) sendTransaction(pk *ecdsa.PrivateKey, toAddr common.Address, toValue *big.Int, data []byte, gasLimit uint64, gasPrice *big.Int, nonce uint64) (common.Hash, error) {
	...
	log.WithField("txHash", signedTx.Hash().Hex()).Info("Transaction confirmed")
...
func (em *ERC20Manager) EtherBalanceOf(pk string) (*big.Int, error) {
	privateKey, err := crypto.HexToECDSA(pk)
	if err != nil {
		return nil, err
	}
...
func (em *ERC20Manager) ERC20BalanceOf(pk string) (string, *big.Int, error) {
	log := logger.GetLogger().WithFields(map[string]interface{}{
		"contract": em.cc.Address,
	})
	cont := common.HexToAddress(em.cc.Address)
	...
	addr := crypto.PubkeyToAddress(privateKey.PublicKey)
	log = log.WithField("address", addr.Hex())
...
func (em *ERC20Manager) ERC20Allowance(pk string) (*big.Int, error) {
	log := logger.GetLogger().WithFields(map[string]interface{}{
		"contract": em.cc.Address,
	})
	cont := common.HexToAddress(em.cc.Address)
