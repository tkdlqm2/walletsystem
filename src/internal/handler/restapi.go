...
func Send(db *gorm.DB) func(c echo.Context) error {
	return func(c echo.Context) error {
		...
		userBalance, err := userBalanceRepo.GetBalanceByUserIDAndCurrencyID(context.Background(), userID, cc.ID)
		if err != nil {
		...
		// withdraw_history 테이블에 레코드 생성
		if err := db.Table("withdraw_history").Create(withdrawInfo).Error; err != nil {
			logger.GetLogger().WithError(err).Error("Failed to create withdraw history record")
			return c.JSON(http.StatusInternalServerError, map[string]string{"error": "Failed to create withdraw record"})
		}
...
func GetCurrency(db *gorm.DB, token string) (*entities.Currency, error) {
	...
	return currencyRepo.GetCurrency(context.Background(), token)
}
