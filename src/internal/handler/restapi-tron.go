...
func TronBalance(db *gorm.DB, email string) (string, error) {
	...
	privateKeys, err := database.GetPrivateKeyUsingEmail(db, email)
	if err != nil {
	...
	encryptedPk, exists := privateKeys["tron"]
	if !exists {
...
func TronCollect(db *gorm.DB, email string) (string, error) {
	...
	if bi.Cmp(big.NewInt(1000000)) < 0 {
		log.Warn("not sended balance less than 1", bi.String())
		return "", errors.New("not sended balance less then 1(" + bi.String() + ")")
	}

	if allow.Cmp(bi) < 0 {
		log.Warn("allowance is less than balance start approve")
		approveFee := big.NewInt(21261000)
		if approveFee.Cmp(trxbi) > 0 {
			log.Warn("depositaddress not enough TRX send approve fee")
	...
	log.WithField("bi", bi.String()).Info("TronCollect succeeded")
...
func GetTronAddress(email, pk string, db *gorm.DB) (string, error) {
	...
	if err != nil && err.Error() == "record not found" {
		...
		log.Info("New chain_account record created for TRON")
	} else if err != nil {
		// 다른 에러 발생
		...
		log.Info("Existing chain_account record updated for TRON")
	}
...
func TronWithdraw(db *gorm.DB, cc *database.Currency, w *database.WithdrawInfo, memo string) (map[string]string, error) {
	log := logger.GetLogger().WithFields(map[string]interface{}{
		"symbol":    cc.Symbol,
	...
	if err != nil {
		...
		database.SendErrMsg(db, "TronWithdraw", err)
		w.Process = "error"
		err = db.Table("withdraw_history").Save(w).Error
		if err != nil {
...
func EncryptAllPrivateKeys(db *gorm.DB) func(c echo.Context) error {
	return func(c echo.Context) error {
		...
		var chainAccounts []database.ChainAccount
		...
		for _, account := range chainAccounts {
			...
			err = db.Model(&database.ChainAccount{}).Where("id = ?", account.ID).Update("private_key", encryptedBase64).Error
			if err != nil {
